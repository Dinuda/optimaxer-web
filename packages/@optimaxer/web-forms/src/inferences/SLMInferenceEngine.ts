...
import { LLMEngine, availableLLMEngines } from "../types/InferenceEngine";
    ...
    static async init(modelName: WebLLMModel | MediaPipeModel, llmInferenceEngine: LLMEngine): Promise<Response> {
        ...
        if (!InferenceEngine.llmInstance) {
            if (llmInferenceEngine === availableLLMEngines.MediaPipe) {
                ...
                InferenceEngine.llmInstance = await MediaPipeInferenceEngine.init(modelName=='gemma'?'gemma-gpu':modelName as MediaPipeModel, false, 2048);
